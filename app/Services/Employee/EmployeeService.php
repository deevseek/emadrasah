...
class EmployeeService
{
    ...
    { return DB::transaction(function () use ($employee, $email, $role) { abort_if(! $employee->is_active, 422, 'Aktifkan pegawai sebelum membuat akun.'); abort_if($employee->user_id, 422, 'Pegawai sudah memiliki akun.'); $user=User::create(['name'=>$employee->fullName(),'email'=>$email,'password'=>Hash::make(Str::password(16)),'is_active'=>true]); $user->assignRole($role); $employee->update(['user_id'=>$user->id,'email'=>$employee->email ?: $email]); $this->log('employee.account-created', $employee, ['akun_baru'=>$user->id,'role'=>$role]); return $user; }); }
