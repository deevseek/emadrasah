...
final class TeachingAssignmentImportService
{
    ...
    public function preview(string $path, int $year, int $semester): array
    {
        ...
        if ($rows === []) {
            throw ValidationException::withMessages(['file' => 'File XLSX tidak menghasilkan baris data. Periksa format workbook dan header.']);
        }

        $missingHeaders = array_values(array_diff(self::REQUIRED_HEADERS, array_keys($rows[0])));
        if ($missingHeaders !== []) {
            throw ValidationException::withMessages(['file' => 'Header XLSX tidak lengkap. Header yang hilang: '.implode(', ', $missingHeaders).'.']);
        }
    ...
    {
        if ($preview === []) {
            throw ValidationException::withMessages(['file' => 'Preview impor kosong. Unggah kembali file XLSX yang berisi data.']);
        }
