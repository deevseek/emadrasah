...
class ScheduleConflictService
{
    public function assertNoConflict(array $data, ?LessonSchedule $ignore = null): void
    {
        ...
        $teacher = ! empty($data['employee_id']) ? (clone $base)->where('employee_id', $data['employee_id'])->first() : null;
        ...
        $dup = LessonSchedule::where('is_active', true)->where('classroom_id',$data['classroom_id'])->where('entry_type',$data['entry_type'] ?? 'lesson')->where('day_of_week',$data['day_of_week'])->where('starts_at',$data['starts_at'])->where('ends_at',$data['ends_at']);
