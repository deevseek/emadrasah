...
class ReportCardService
{
        ...
        return DB::transaction(function () use ($enrollment, $semesterId): ReportCard {
            ...
            if ($card->status === 'locked') {
                throw ValidationException::withMessages([
                    'status' => 'Rapor terkunci tidak dapat digenerate ulang.',
                ]);
            }

            $card->forceFill(
                $this->attendanceSnapshot($enrollment) + ['generated_at' => now()]
            )->save();

            ReportCardSubject::where('report_card_id', $card->id)->delete();

            $subjectIds = AssessmentComponent::where('classroom_id', $enrollment->classroom_id)
            ...
            foreach ($subjectIds as $index => $subjectId) {
                ...
                $result = $this->assessmentService->finalScore(
            ...
            ReportCardBtaq::updateOrCreate(
                ['report_card_id' => $card->id],
                [
                    'final_score' => $lastBtaq?->reading_score,
                    'predicate' => $this->assessmentService->predicate(
                        $lastBtaq?->reading_score !== null
                            ? (float) $lastBtaq->reading_score
                            : null
                    ),
            ...
            activity('report-card')
                ->performedOn($card)
                ->causedBy(auth()->user())
                ->event('report-card.generated')
                ->log('Rapor digenerate');
    ...
    public function transition(ReportCard $card, string $status, ?string $reason = null): void
    {
        DB::transaction(function () use ($card, $status, $reason): void {
            if ($card->status === 'locked' && $status !== 'reopened') {
                throw ValidationException::withMessages([
                    'status' => 'Rapor terkunci.',
                ]);
            }

            if ($status === 'reopened' && blank($reason)) {
                throw ValidationException::withMessages([
                    'reason' => 'Alasan reopen wajib diisi.',
                ]);
            }
            ...
            if ($status === 'reopened') {
                ...
                $updates['reopened_by'] = auth()->id();
            }

            $card->update($updates);

            ReportCardStatusHistory::create([
                'report_card_id' => $card->id,
                'from_status' => $from,
                'to_status' => $status,
                'reason' => $reason,
            ...
            activity('report-card')
                ->performedOn($card)
                ->causedBy(auth()->user())
                ->event('report-card.status')
                ->log('Status rapor diperbarui');
        });
    }

    private function attendanceSnapshot(StudentEnrollment $enrollment): array
    {
        $query = StudentAttendance::where('student_id', $enrollment->student_id)
