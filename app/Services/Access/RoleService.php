...
class RoleService
{
    private function normalized(array $permissions): array { $allowed = collect(config('permissions'))->flatMap(fn (array $group) => array_keys($group['permissions'])); $p=collect($permissions)->intersect($allowed)->unique(); if($p->contains(fn($x)=>str_starts_with($x,'users.')&&$x!=='users.view'))$p->push('users.view'); if($p->contains(fn($x)=>str_starts_with($x,'roles.')&&$x!=='roles.view'))$p->push('roles.view'); if($p->contains(fn($x)=>str_starts_with($x,'academic-periods.')&&$x!=='academic-periods.view'))$p->push('academic-periods.view'); if($p->contains(fn($x)=>str_starts_with($x,'personnel.')&&$x!=='personnel.view'))$p->push('personnel.view'); if($p->contains(fn($x)=>str_starts_with($x,'students.')&&$x!=='students.view'))$p->push('students.view'); if($p->contains(fn($x)=>str_starts_with($x,'classrooms.')&&!in_array($x,['classrooms.view','classrooms.view-own'],true)))$p->push('classrooms.view'); if($p->contains(fn($x)=>str_starts_with($x,'subjects.')&&$x!=='subjects.view'))$p->push('subjects.view'); if($p->contains('teaching-assignments.print-own'))$p->push('teaching-assignments.view-own'); if($p->contains(fn($x)=>str_starts_with($x,'teaching-assignments.')&&!in_array($x,['teaching-assignments.view','teaching-assignments.view-own','teaching-assignments.print-own'],true)))$p->push('teaching-assignments.view'); return $p->unique()->all(); }
