...
class RoleService
{
    private function normalized(array $permissions): array { $allowed = collect(config('permissions'))->flatMap(fn (array $group) => array_keys($group['permissions'])); $p=collect($permissions)->intersect($allowed)->unique(); if($p->contains(fn($x)=>str_starts_with($x,'users.')&&$x!=='users.view'))$p->push('users.view'); if($p->contains(fn($x)=>str_starts_with($x,'roles.')&&$x!=='roles.view'))$p->push('roles.view'); if($p->contains(fn($x)=>str_starts_with($x,'academic-periods.')&&$x!=='academic-periods.view'))$p->push('academic-periods.view'); return $p->unique()->all(); }
