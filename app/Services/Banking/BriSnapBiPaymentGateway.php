...
class BriSnapBiPaymentGateway implements BankPaymentGateway
{
    ...
    public function createVirtualAccount(array $request): array
    {
        ...
        $partnerServiceId = (string) ($this->configuration->setting()?->partner_service_id ?: config('bri.briva.institution_code'));
        ...
        $prefix = (string) ($this->configuration->setting()?->customer_number_prefix ?? '');
        ...
        return [
            'status' => 'active',
            'external_id' => (string) ($request['external_id'] ?? Str::uuid()),
            'customer_no' => $customerNo,
            'partner_service_id' => $service,
            'virtual_account_number' => $service.$customerNo,
        ];
    }

    public function transfer(array $request): array
    {
        throw new RuntimeException('Gunakan BankTransferGateway untuk transfer dana.');
    }

    public function inquire(string $id): array
    {
        return ['status' => 'active', 'external_id' => $id];
    }
}
