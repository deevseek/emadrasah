...
class RfidWriterController extends Controller {
 public function store(IssueRfidCardRequest $r,Student $student,RfidWriterService $s):JsonResponse{$c=$s->issue($student,$r->user(),$r->operation());return response()->json(['success'=>true,'command_id'=>$c->id,'status'=>$c->status->value,'operation'=>$c->payload['operation']??'write']);}
