...
use App\Http\Controllers\Controller; use App\Http\Requests\Rfid\IssueRfidCardRequest; use App\Models\{RfidDeviceCommand,Student}; use App\Services\Rfid\RfidWriterService; use Illuminate\Http\{JsonResponse,Request};
class RfidWriterController extends Controller {
 public function store(IssueRfidCardRequest $r,Student $student,RfidWriterService $s):JsonResponse{$c=$s->issue($student,$r->user(),$r->boolean('replace'));return response()->json(['success'=>true,'command_id'=>$c->id,'status'=>$c->status->value,'operation'=>$c->payload['operation']??'write']);}
