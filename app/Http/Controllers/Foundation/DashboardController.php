...
class DashboardController extends Controller
{
    public function __invoke(): View
    {
        ...
        $classrooms = Classroom::with([
            'gradeLevel',
            'studentEnrollments' => fn ($query) => $query->where('enrollment_status', 'active'),
        ...
        return view('dashboard', [
            ...
            'stats' => [
                ...
                'Guru hadir hari ini' => EmployeeAttendance::whereDate('attendance_date', today())
                    ->where('status', AttendanceStatus::Present->value)
                    ->count(),
                'Guru terlambat' => EmployeeAttendance::whereDate('attendance_date', today())
                    ->where('status', AttendanceStatus::Late->value)
                    ->count(),
                'Guru izin/sakit' => EmployeeAttendance::whereDate('attendance_date', today())
                    ->whereIn('status', [AttendanceStatus::Leave->value, AttendanceStatus::Sick->value])
                    ->count(),
                'Siswa hadir' => StudentAttendance::whereDate('attendance_date', today())
                    ->where('status', AttendanceStatus::Present->value)
                    ->count(),
                'Siswa izin/sakit/alpha' => StudentAttendance::whereDate('attendance_date', today())
                    ->whereIn('status', [
                ...
                'Siswa perlu bimbingan' => BtaqJournalStudent::where('progress_status', 'needs_guidance')
                    ->distinct('student_id')
                    ->count(),
            ...
            'latestLogins' => LoginHistory::where('successful', true)
                ->latest('attempted_at')
                ->limit(5)
                ->get(),
            'latestActivities' => ActivityLog::latest()->limit(5)->get(),
            'latestStudents' => Student::latest()->limit(5)->get(),
            'latestEnrollments' => StudentEnrollment::with(['student', 'classroom', 'academicYear'])
                ->latest()
                ->limit(5)
                ->get(),
            'studentsByGrade' => GradeLevel::withCount([
                'classrooms as active_students_count' => fn ($query) => $query
                    ->join(
                        'student_enrollments',
                        'classrooms.id',
                        '=',
