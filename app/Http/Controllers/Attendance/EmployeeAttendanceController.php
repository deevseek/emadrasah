...
final class EmployeeAttendanceController extends Controller
{
    ...
    {
        $records = EmployeeAttendance::with(['employee', 'workSchedule'])->filter(request()->only(['date','from','to','month','status','employee_id','verification_status','q']))->latest('attendance_date')->get();
        return Response::streamDownload(function () use ($records): void { $out = fopen('php://output', 'w'); fputcsv($out, ['Pegawai','Tanggal','Jadwal','Check-in','Check-out','Status','Terlambat','Pulang awal','Verifikasi']); foreach ($records as $record) fputcsv($out, [$record->employee?->name, $record->attendance_date?->toDateString(), $record->workSchedule?->name, $record->checked_in_at?->format('H:i'), $record->checked_out_at?->format('H:i'), $record->status?->label(), $record->late_minutes, $record->early_leave_minutes, $record->verification_status?->value]); fclose($out); }, 'kehadiran-pegawai.csv', ['Content-Type' => 'text/csv']);
    }
}
