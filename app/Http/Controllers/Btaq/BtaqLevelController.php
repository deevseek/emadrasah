...
class BtaqLevelController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse { BtaqLevel::create($request->validate(['code'=>'required|string|max:50|unique:btaq_levels,code','name'=>'required|string','sequence'=>'required|integer','description'=>'nullable|string','is_active'=>'boolean'])); activity('btaq')->event('btaq-level.created')->log('Level BTAQ dibuat'); return redirect()->route('btaq-levels.index')->with('status','Level BTAQ disimpan.'); }
    ...
    public function update(Request $request, BtaqLevel $btaqLevel): RedirectResponse { $btaqLevel->update($request->validate(['code'=>'required|string|max:50|unique:btaq_levels,code,'.$btaqLevel->id,'name'=>'required|string','sequence'=>'required|integer','description'=>'nullable|string','is_active'=>'boolean'])); activity('btaq')->performedOn($btaqLevel)->event('btaq-level.updated')->log('Level BTAQ diperbarui'); return redirect()->route('btaq-levels.show',$btaqLevel)->with('status','Level BTAQ diperbarui.'); }
