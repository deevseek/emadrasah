...
class BtaqMaterialController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse { BtaqMaterial::create($request->validate(['btaq_level_id'=>'required|exists:btaq_levels,id','code'=>'required|unique:btaq_materials,code','name'=>'required','category'=>'required','sequence'=>'required|integer','target_description'=>'nullable','is_active'=>'boolean'])); return redirect()->route('btaq-materials.index')->with('status','Materi BTAQ disimpan.'); }
    ...
    public function update(Request $request, BtaqMaterial $btaqMaterial): RedirectResponse { $btaqMaterial->update($request->validate(['btaq_level_id'=>'required|exists:btaq_levels,id','code'=>'required|unique:btaq_materials,code,'.$btaqMaterial->id,'name'=>'required','category'=>'required','sequence'=>'required|integer','target_description'=>'nullable','is_active'=>'boolean'])); return redirect()->route('btaq-materials.show',$btaqMaterial)->with('status','Materi diperbarui.'); }
}
