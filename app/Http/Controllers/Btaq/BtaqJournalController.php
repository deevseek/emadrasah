...
class BtaqJournalController extends Controller
{
    ...
    public function submit(BtaqJournal $btaqJournal): RedirectResponse { $btaqJournal->update(['status'=>'submitted','submitted_at'=>now()]); activity('btaq')->performedOn($btaqJournal)->log('Jurnal BTAQ diajukan'); return back()->with('status','Jurnal diajukan.'); }
    public function verify(BtaqJournal $btaqJournal): RedirectResponse { $btaqJournal->update(['status'=>'verified','verified_at'=>now(),'verified_by'=>auth()->id()]); activity('btaq')->performedOn($btaqJournal)->log('Jurnal BTAQ diverifikasi'); return back()->with('status','Jurnal diverifikasi.'); }
    public function reject(Request $request, BtaqJournal $btaqJournal): RedirectResponse { $data=$request->validate(['rejection_reason'=>'required|string']); $btaqJournal->update(['status'=>'rejected','rejection_reason'=>$data['rejection_reason']]); return back()->with('status','Jurnal ditolak.'); }
