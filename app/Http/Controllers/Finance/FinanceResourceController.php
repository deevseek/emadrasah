...
final class FinanceResourceController extends Controller
{
    ...
    public function discounts(): View { return view('finance.discounts.index', ['discounts' => StudentDiscount::with('student','feeType')->latest()->paginate(15), 'students' => Student::orderBy('name')->get(), 'feeTypes' => \App\Models\Finance\FeeType::where('is_active', true)->get()]); }
    ...
    public function cancelDiscount(Request $request, StudentDiscount $studentDiscount): RedirectResponse { $studentDiscount->update(['status' => ApprovalStatus::Cancelled->value, 'cancelled_by' => auth()->id(), 'cancelled_at' => now(), 'cancellation_reason' => $request->validate(['reason' => 'required|string|min:5'])['reason']]); return back()->with('status','Potongan dibatalkan.'); }
    public function arrears(Request $request): View { $items = StudentInvoice::with('student.activeEnrollment.classroom','feeType')->where('outstanding_amount','>',0)->whereDate('due_on','<',today())->where('status','!=','cancelled')->when($request->classroom_id, fn($q,$id)=>$q->where('classroom_id',$id))->latest('due_on')->paginate(20); return view('finance.arrears.index', ['items'=>$items, 'classrooms'=>\App\Models\Classroom::orderBy('name')->get()]); }
    public function exportArrears(Request $request): \Symfony\Component\HttpFoundation\StreamedResponse { return response()->streamDownload(function (): void { echo "Nomor,Siswa,Sisa
"; StudentInvoice::with('student')->where('outstanding_amount','>',0)->whereDate('due_on','<',today())->each(fn($bill)=>print($bill->invoice_number.','.$bill->student?->name.','.$bill->outstanding_amount."
")); }, 'tunggakan-siswa.csv', ['Content-Type'=>'text/csv']); }
    public function printArrears(Request $request): View { return view('finance.arrears.print', ['items'=>StudentInvoice::with('student.activeEnrollment.classroom','feeType')->where('outstanding_amount','>',0)->whereDate('due_on','<',today())->get()]); }
    public function studentReports(Request $request): View { return view('finance.reports.student', ['payments'=>\App\Models\Finance\StudentPayment::with('student','allocations.invoice.feeType')->where('status','posted')->latest('payment_date')->paginate(20), 'summary'=>['payments'=>\App\Models\Finance\StudentPayment::where('status','posted')->sum('total_amount'), 'outstanding'=>StudentInvoice::where('outstanding_amount','>',0)->sum('outstanding_amount'), 'discounts'=>StudentDiscount::sum('discount_value'), 'cancelled'=>\App\Models\Finance\StudentPayment::where('status','cancelled')->sum('total_amount')]]); }
    public function exportStudentReports(Request $request): \Symfony\Component\HttpFoundation\StreamedResponse { return response()->streamDownload(function (): void { echo "Nomor,Tanggal,Siswa,Nominal,Status
"; \App\Models\Finance\StudentPayment::with('student')->latest('payment_date')->each(fn($p)=>print($p->payment_number.','.$p->payment_date.','.$p->student?->name.','.$p->total_amount.','.$p->status."
")); }, 'laporan-keuangan-siswa.csv', ['Content-Type'=>'text/csv']); }
    public function printStudentReports(Request $request): View { return view('finance.reports.print-student', ['payments'=>\App\Models\Finance\StudentPayment::with('student')->where('status','posted')->latest('payment_date')->get()]); }
