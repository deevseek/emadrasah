...
@php
    ...
    $navGroups = [
        ...
        ['label' => 'Inventaris', 'items' => [
            ['label' => 'Dashboard Inventaris', 'route' => 'inventory.dashboard', 'match' => 'inventory.dashboard', 'permission' => 'inventory.view'],
            ['label' => 'Data Barang', 'route' => 'inventory.items.index', 'match' => 'inventory.items.*', 'permission' => 'inventory.view'],
            ['label' => 'Kategori Barang', 'route' => 'inventory.categories.index', 'match' => 'inventory.categories.*', 'permission' => 'inventory.manage-master'],
            ['label' => 'Lokasi & Ruangan', 'route' => 'inventory.locations.index', 'match' => 'inventory.locations.*', 'permission' => 'inventory.manage-master'],
            ['label' => 'Kondisi Barang', 'route' => 'inventory.conditions.index', 'match' => 'inventory.conditions.*', 'permission' => 'inventory.manage-master'],
            ['label' => 'Transaksi Inventaris', 'route' => 'inventory.transactions.index', 'match' => 'inventory.transactions.*', 'permission' => ['inventory.view','inventory.adjust','inventory.transfer','inventory.change-condition']],
            ['label' => 'Stock Opname', 'route' => 'inventory.stock-opnames.index', 'match' => 'inventory.stock-opnames.*', 'permission' => 'inventory.stock-opname'],
            ['label' => 'Laporan Inventaris', 'route' => 'inventory.reports.index', 'match' => 'inventory.reports.*', 'permission' => ['inventory.report','inventory.print','inventory.export']],
        ]],
    ...
    $breadcrumbMap = [
        ...
        'inventory.dashboard' => ['Beranda', 'Inventaris', 'Dashboard Inventaris'],
        'inventory.items.*' => ['Beranda', 'Inventaris', 'Data Barang'],
        'inventory.categories.*' => ['Beranda', 'Inventaris', 'Kategori Barang'],
        'inventory.locations.*' => ['Beranda', 'Inventaris', 'Lokasi dan Ruangan'],
        'inventory.conditions.*' => ['Beranda', 'Inventaris', 'Kondisi Barang'],
        'inventory.transactions.*' => ['Beranda', 'Inventaris', 'Transaksi Inventaris'],
        'inventory.stock-opnames.*' => ['Beranda', 'Inventaris', 'Stock Opname'],
        'inventory.reports.*' => ['Beranda', 'Inventaris', 'Laporan Inventaris'],
    ...
    foreach ($breadcrumbMap as $pattern => $labels) {
        if (request()->routeIs($pattern)) { $crumbs = collect($labels); break; }
    }
    ...
    @foreach($navGroups as $group)
      ...
      @if($visible->isNotEmpty())
        <div><p class="sidebar-section px-3 pb-2 text-xs font-bold uppercase tracking-wider text-emerald-200/80">{{ $group['label'] }}</p><div class="space-y-1">
          @foreach($visible as $item)@php($isActive = request()->routeIs($item['match']) && ! (isset($item['exclude']) && request()->routeIs($item['exclude'])))<a href="{{ route($item['route']) }}" onclick="closeMobileSidebar()" @class(['nav-link','nav-link-active'=>$isActive])><svg class="h-5 w-5 shrink-0" viewBox="0 0 24 24" fill="currentColor"><path d="M4 5a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V5Zm4 2v2h8V7H8Zm0 4v2h8v-2H8Zm0 4v2h5v-2H8Z"/></svg><span class="sidebar-label flex-1 truncate">{{ $item['label'] }}</span></a>@endforeach
        </div></div>
      @endif
    @endforeach
  </nav>
...
<div class="page-container p-4 sm:p-6">@if($displayFlash && session('status'))<div class="mb-6 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-medium text-emerald-800">{{ session('status') }}</div>@endif @if($displayFlash && $errors->any())<div class="mb-6 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700">Periksa kembali isian formulir.</div>@endif {{ $slot }}</div></main>
</body></html>
