...
<aside class="app-sidebar">
  ...
  <div class="border-t border-white/10 p-4"><div class="flex items-center gap-3 rounded-2xl bg-white/10 p-3"><div class="grid h-10 w-10 place-items-center rounded-xl bg-amber-300 font-bold text-emerald-950">{{ str($user?->name ?? 'U')->substr(0,2)->upper() }}</div><div class="min-w-0 sidebar-user-detail"><p class="truncate text-sm font-semibold">{{ $user?->name }}</p><p class="truncate text-xs text-emerald-100">{{ $role }}</p><div class="mt-2 flex flex-wrap gap-2 text-xs">@if($hasPersonnelProfile)<a href="{{ route('personnel.profile.show') }}" class="text-amber-200 hover:text-white">Profil saya</a>@endif<a href="{{ route('password.change') }}" class="text-amber-200 hover:text-white">Ganti password</a><form method="post" action="{{ route('logout') }}">@csrf<button class="text-amber-200 hover:text-white">Keluar</button></form></div></div></div></div>
</aside>
<main class="app-main"><header class="sticky top-0 z-20 border-b border-slate-200 bg-white/90 backdrop-blur"><div class="flex flex-col gap-4 px-4 py-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between"><div class="flex min-w-0 items-start gap-3"><button type="button" class="rounded-xl border border-slate-200 bg-white p-2 focus:outline-none focus:ring-2 focus:ring-emerald-500 lg:hidden" onclick="openMobileSidebar()" aria-label="Buka navigasi">☰</button><button type="button" class="hidden rounded-xl border border-slate-200 bg-white p-2 focus:outline-none focus:ring-2 focus:ring-emerald-500 lg:block" onclick="toggleSidebar()" aria-label="Ringkas navigasi">☰</button><div class="min-w-0"><p class="text-xs text-slate-500">{{ $crumbs->join(' / ') }}</p><h1 class="text-xl font-bold text-emerald-950 sm:text-2xl">{{ $title }}</h1></div></div><div class="flex flex-wrap items-center gap-2 text-xs text-slate-600"><div class="relative" data-notification-center data-url="{{ route('notifications.latest') }}" data-read-url="{{ route('notifications.read-all') }}" data-interval="{{ config('notifications.poll_interval', 10) * 1000 }}"><button type="button" data-notification-toggle class="relative grid h-9 w-9 place-items-center rounded-xl border border-slate-200 bg-white text-emerald-800 hover:bg-emerald-50" aria-label="Buka notifikasi" aria-expanded="false">🔔<span data-notification-badge class="absolute -right-1 -top-1 hidden min-w-5 rounded-full bg-rose-600 px-1 text-center text-[10px] font-bold leading-5 text-white"></span></button><div data-notification-panel class="absolute right-0 mt-2 hidden w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl"><div class="flex items-center justify-between border-b border-slate-100 px-4 py-3"><strong class="text-sm text-emerald-950">Notifikasi</strong><button type="button" data-notification-read-all class="text-xs font-semibold text-emerald-700 hover:underline">Tandai dibaca</button></div><div data-notification-list class="max-h-80 overflow-y-auto"><p class="p-4 text-sm text-slate-500">Memuat notifikasi…</p></div><a href="{{ route('notifications.index') }}" class="block border-t border-slate-100 px-4 py-3 text-center text-xs font-bold text-emerald-700 hover:bg-emerald-50">Lihat semua notifikasi</a></div></div><span class="rounded-full bg-emerald-50 px-3 py-1.5 font-semibold text-emerald-700">{{ $activeAcademicPeriod?->name ?? 'Tahun ajaran belum diatur' }}</span><span class="rounded-full bg-amber-50 px-3 py-1.5 font-semibold text-amber-700">{{ $activeAcademicPeriod?->activeSemester?->display_name ?? 'Semester belum diatur' }}</span><span>{{ now()->translatedFormat('d F Y') }}</span></div></div></header>
