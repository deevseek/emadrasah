<x-layouts.app :title="$title" :breadcrumbs="['Beranda','Kesiswaan','Data Siswa',$student->full_name]"><div class="space-y-5"><x-ui.page-header :title="$student->full_name" :description="$student->age_label"><div class="flex gap-2">@can('students.update')<a class="btn btn-primary" href="{{ route('students.edit',$student) }}">Edit</a>@endcan @can('students.change-status')<a class="btn btn-secondary" href="{{ route('students.status.edit',$student) }}">Ubah Status</a>@endcan<a class="btn btn-secondary" href="{{ route('students.index') }}">Kembali</a></div></x-ui.page-header><x-ui.card><div class="flex flex-col gap-5 sm:flex-row sm:items-center">@if($student->photo_path)<img class="h-48 w-40 rounded-2xl border border-slate-200 object-cover" data-student-photo-preview src="{{ route('students.photo',$student) }}" alt="Foto {{ $student->full_name }}">@else<div class="flex h-48 w-40 items-center justify-center rounded-2xl bg-slate-100 text-4xl font-bold text-emerald-900" data-student-photo-placeholder>{{ $student->initials?:'?' }}</div><img class="hidden h-48 w-40 rounded-2xl border border-slate-200 object-cover" data-student-photo-preview alt="Pratinjau foto siswa">@endif<div class="flex-1"><h2 class="text-lg font-bold text-emerald-950">Foto Siswa</h2><p class="mt-1 text-sm text-slate-500">@if($student->photo_path)Foto tersimpan secara aman dan hanya dapat dilihat pengguna yang berwenang.@else Belum ada foto siswa.@endif</p>@can('students.photo.manage')<form method="post" enctype="multipart/form-data" action="{{ route('students.photo.update',$student) }}" class="mt-4 space-y-2">@csrf<label class="block"><span class="label">Ambil Foto dengan Kamera Belakang</span><input class="input" name="photo" type="file" accept="image/jpeg,image/png,image/webp" capture="environment" required data-student-photo-input><span class="mt-1 block text-xs text-slate-500">Wajah harus terlihat jelas. Foto yang lebih besar dari 5 MB akan dikompresi otomatis, lalu disimpan di server dengan nama NISN siswa.</span><span class="mt-1 block text-xs font-semibold text-emerald-700" role="status" aria-live="polite" data-student-photo-status></span></label>@error('photo')<p class="text-sm text-red-600">{{ $message }}</p>@enderror<button class="btn btn-primary" type="submit" @disabled(blank($student->nisn))>Simpan Foto</button>@if(blank($student->nisn))<p class="text-sm text-red-600">NISN belum tersedia. Minta operator melengkapi NISN terlebih dahulu.</p>@endif</form>@endcan</div></div></x-ui.card><x-ui.card><div class="grid gap-4 md:grid-cols-2">@foreach(['gender_label'=>'Jenis Kelamin','nisn'=>'NISN','nik'=>'NIK','birth_place'=>'Tempat Lahir','birth_date'=>'Tanggal Lahir','classroom_label'=>'Tingkat/Rombel','status_label'=>'Status','address'=>'Alamat','phone'=>'Nomor Telepon','special_needs'=>'Kebutuhan Khusus','disability'=>'Disabilitas','kip_pip_number'=>'Nomor KIP/PIP','father_name'=>'Nama Ayah','mother_name'=>'Nama Ibu','guardian_name'=>'Nama Wali'] as $key=>$label)<div><p class="text-xs text-slate-500">{{ $label }}</p><p class="font-semibold">@if($key==='birth_date'){{ $student->birth_date?->translatedFormat('d F Y')?:'—' }}@elseif(in_array($key,['nik','address','phone','kip_pip_number'])){{ $display[$key]?:'—' }}@elseif(in_array($key,['special_needs','disability'])){{ $student->$key?:'Tidak Ada' }}@else{{ $student->$key?:'—' }}@endif</p></div>@endforeach</div></x-ui.card><section class="rounded-xl border border-slate-200 bg-white p-4">
    <div class="flex flex-wrap items-start justify-between gap-3">
        ...
        <div class="flex flex-wrap gap-2">
            @if($student->activeRfidCard)
                @can('rfid-card.replace')<button class="btn btn-primary" type="button" data-rfid-write data-operation="rewrite" @disabled(!$writerEnabled || !$writerOnline)>Tulis Ulang Kartu</button>@endcan
            ...
            @else
                @can('rfid-card.issue')<button class="btn btn-primary" type="button" data-rfid-write data-operation="write" @disabled(!$writerEnabled || !$writerOnline)>Tulis Kartu</button>@endcan
            @endif
            @can('rfid-card.replace')<button class="btn btn-secondary" type="button" data-rfid-write data-operation="reassign" @disabled(!$writerEnabled || !$writerOnline)>Gunakan Kartu Lama</button>@endcan
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('[data-rfid-write]').forEach((button) => {
        button.addEventListener('click', async () => {
            ...
            isRewrite = operation !== 'write';
            if (operation === 'rewrite' && ! window.confirm('Tulis ulang kartu RFID siswa ini?\n\nTempel kartu ke RFID Writer satu kali. Data card_token lama pada kartu akan langsung ditimpa dengan card_token baru. Tidak perlu menghapus kartu terlebih dahulu.')) return;
            if (operation === 'reassign' && ! window.confirm('Gunakan kartu lama untuk siswa ini?\n\nJika kartu masih terdaftar pada siswa lain, assignment lama akan dinonaktifkan setelah kartu berhasil diverifikasi.')) return;
