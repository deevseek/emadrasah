    ...
    @if($personnel->is_active)
        <dialog id="face-enrollment" class="m-auto w-full max-w-xl rounded-2xl p-6 backdrop:bg-slate-950/60">
            <form method="post" enctype="multipart/form-data" action="{{ route('personnel.profile.face.enroll') }}">
                ...
                <x-face-camera-guide class="mt-4" video-id="face-video" status-id="face-guide-status" />
                ...
                @foreach(['front_1', 'front_2', 'natural', 'left', 'right'] as $pose)<input type="file" name="{{ $pose }}" accept="image/jpeg,image/png" hidden required>@endforeach
        ...
        @push('scripts')
            <script>
                (() => {
                    const dialog = document.getElementById('face-enrollment'), open = document.getElementById('face-enrollment-open'), video = document.getElementById('face-video'), canvas = document.getElementById('face-canvas'), capture = document.getElementById('face-capture'), submit = document.getElementById('face-submit'), step = document.getElementById('face-step'), error = document.getElementById('face-camera-error'), guideStatus = document.getElementById('face-guide-status');
                    const poses = ['front_1', 'front_2', 'natural', 'left', 'right'], directions = ['depan (netral)', 'tetap ke depan', 'depan dengan ekspresi natural', 'sedikit ke kiri', 'sedikit ke kanan']; let current = 0;
                    const stop = () => { video?.srcObject?.getTracks().forEach(track => track.stop()); if (video) video.srcObject = null; };
                    const fail = message => { error.textContent = message; error.classList.remove('hidden'); video.closest('[data-face-camera]').dataset.guideState = 'error'; guideStatus.textContent = 'Kamera tidak siap.'; capture.disabled = true; };
                    open?.addEventListener('click', async () => { current = 0; dialog.querySelectorAll('input[type="file"]').forEach(input => input.value = ''); step.textContent = 'Foto 1: wajah menghadap depan'; guideStatus.textContent = 'Posisikan wajah di dalam area.'; video.closest('[data-face-camera]').dataset.guideState = 'ready'; capture.classList.remove('hidden'); submit.classList.add('hidden'); capture.disabled = true; dialog.showModal(); error.classList.add('hidden'); if (!window.isSecureContext && !['localhost', '127.0.0.1'].includes(location.hostname)) return fail('Kamera hanya dapat digunakan melalui koneksi HTTPS.'); if (!navigator.mediaDevices?.getUserMedia) return fail('Peramban ini tidak mendukung akses kamera.'); try { const stream = await navigator.mediaDevices.getUserMedia({video: {facingMode: 'user', width: {ideal: 1280}, height: {ideal: 720}}, audio: false}); video.srcObject = stream; await video.play(); capture.disabled = false; } catch (exception) { fail(exception.name === 'NotAllowedError' ? 'Izin kamera ditolak. Izinkan akses kamera lalu coba lagi.' : 'Kamera tidak dapat dibuka. Pastikan kamera tersedia.'); } });
                    dialog?.querySelector('[data-face-close]')?.addEventListener('click', () => dialog.close()); dialog?.addEventListener('close', stop); window.addEventListener('pagehide', stop);
                    capture?.addEventListener('click', () => { if (!video.videoWidth || !video.videoHeight) return fail('Kamera belum siap. Tunggu beberapa saat lalu coba lagi.'); const scale = Math.min(1, 1280 / Math.max(video.videoWidth, video.videoHeight)); canvas.width = Math.round(video.videoWidth * scale); canvas.height = Math.round(video.videoHeight * scale); canvas.getContext('2d').drawImage(video, 0, 0); canvas.toBlob(blob => { if (!blob) return fail('Foto gagal diambil. Silakan coba lagi.'); const input = dialog.querySelector(`[name="${poses[current]}"]`), transfer = new DataTransfer(); transfer.items.add(new File([blob], `${poses[current]}.jpg`, {type: 'image/jpeg'})); input.files = transfer.files; current++; if (current === 5) { capture.classList.add('hidden'); submit.classList.remove('hidden'); step.textContent = 'Lima foto siap divalidasi dan disimpan.'; stop(); } else step.textContent = `Foto ${current + 1}: wajah ${directions[current]}`; }, 'image/jpeg', .88); });
                })();
            </script>
        @endpush
    @endif
