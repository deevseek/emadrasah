...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('btaq_journals', function (Blueprint $table): void {
            ...
            $table->timestamps();
            $table->unique(
                ['btaq_group_id', 'journal_date', 'session_number'],
                'btaq_journal_session_unique'
            );
        });
        ...
        Schema::create('student_scores', function (Blueprint $table): void {
            ...
            $table->timestamps();
            $table->unique(
                ['assessment_component_id', 'student_id'],
                'score_component_student_unique'
            );
        });
        ...
        Schema::create('report_cards', function (Blueprint $table): void {
            ...
            $table->unsignedSmallInteger('permission_count')->default(0);
            ...
            $table->unique(
                ...
                'report_student_period_unique'
            );
        });

        Schema::create('report_card_subjects', function (Blueprint $table): void {
            $table->id();
            ...
            $table->unsignedSmallInteger('sequence');
            $table->timestamps();
            $table->unique(['report_card_id', 'subject_id'], 'report_subject_unique');
        });

        Schema::create('report_card_btaq', function (Blueprint $table): void {
            $table->id();
            $table->foreignId('report_card_id')->unique()->constrained()->cascadeOnDelete();
            $table->foreignId('btaq_level_id')->nullable()->constrained()->nullOnDelete();
            $table->foreignId('last_material_id')->nullable()->constrained('btaq_materials')->nullOnDelete();
            $table->decimal('final_score', 5, 2)->nullable();
            $table->string('predicate')->nullable();
            $table->text('achievement_description')->nullable();
            $table->text('development_notes')->nullable();
            $table->boolean('needs_guidance')->default(false);
            $table->timestamps();
        });

        Schema::create('report_card_status_histories', function (Blueprint $table): void {
            $table->id();
            $table->foreignId('report_card_id')->constrained()->cascadeOnDelete();
        ...
        Schema::create('student_attitude_notes', function (Blueprint $table): void {
            $table->id();
            $table->foreignId('student_id')->constrained()->cascadeOnDelete();
            $table->foreignId('classroom_id')->constrained()->cascadeOnDelete();
            $table->foreignId('academic_year_id')->constrained()->cascadeOnDelete();
            $table->foreignId('semester_id')->constrained()->cascadeOnDelete();
            $table->text('spiritual_notes')->nullable();
            ...
            $table->unique(
                ['student_id', 'academic_year_id', 'semester_id'],
                'student_attitude_period_unique'
            );
        });
        ...
        Schema::create('student_extracurriculars', function (Blueprint $table): void {
            $table->id();
            $table->foreignId('student_id')->constrained()->cascadeOnDelete();
            ...
            $table->timestamps();
            $table->unique(
                ['student_id', 'extracurricular_id', 'academic_year_id', 'semester_id'],
                'student_extra_unique'
            );
        });
