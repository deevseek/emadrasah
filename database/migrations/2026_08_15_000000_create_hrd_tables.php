...
return new class extends Migration {
    ...
    {
        $this->addPersonnelColumnIfMissing('employment_start_date', fn (Blueprint $table) => $table->date('employment_start_date')->nullable()->after('employment_status'));
        $this->addPersonnelColumnIfMissing('base_salary', fn (Blueprint $table) => $table->decimal('base_salary', 15, 2)->nullable()->after('bank_account_number'));
        $this->addPersonnelColumnIfMissing('default_shift_number', fn (Blueprint $table) => $table->unsignedTinyInteger('default_shift_number')->default(1)->after('base_salary'));
        $this->addPersonnelColumnIfMissing('payroll_enabled', fn (Blueprint $table) => $table->boolean('payroll_enabled')->default(true)->after('default_shift_number'));
        $this->addPersonnelColumnIfMissing('face_reference_path', fn (Blueprint $table) => $table->string('face_reference_path')->nullable()->after('payroll_enabled'));
        $this->createTableIfMissing('personnel_attendances', function (Blueprint $table): void {
        ...
        $this->createTableIfMissing('personnel_attendance_late_remarks', function (Blueprint $table): void {
        ...
        $this->createTableIfMissing('personnel_leave_requests', function (Blueprint $table): void {
        ...
        $this->createTableIfMissing('personnel_payrolls', function (Blueprint $table): void {
        ...
        $this->createTableIfMissing('personnel_cash_advances', function (Blueprint $table): void {
        ...
        $this->createTableIfMissing('personnel_cash_advance_payments', function (Blueprint $table): void {
            $table->id(); $table->foreignId('cash_advance_id')->constrained('personnel_cash_advances')->restrictOnDelete(); $table->foreignId('payroll_id')->nullable()->constrained('personnel_payrolls')->restrictOnDelete(); $table->date('payment_date'); $table->decimal('amount', 15, 2); $table->string('method')->index(); $table->text('note')->nullable(); $table->foreignId('created_by')->nullable()->constrained('users')->nullOnDelete(); $table->timestamps(); $table->unique(['cash_advance_id', 'payroll_id'], 'cash_advance_payment_payroll_unique');
        });
    }
    public function down(): void
    {
        ...
        $columns = array_values(array_filter(
            ['employment_start_date', 'base_salary', 'default_shift_number', 'payroll_enabled', 'face_reference_path'],
            fn (string $column): bool => Schema::hasColumn('personnel', $column),
        ));

        if ($columns !== []) {
            Schema::table('personnel', fn (Blueprint $table) => $table->dropColumn($columns));
        }
    }

    private function addPersonnelColumnIfMissing(string $column, Closure $definition): void
    {
        if (Schema::hasColumn('personnel', $column)) {
            return;
        }

        Schema::table('personnel', $definition);
    }
